...
After running "run streamlit run pychain.py" form command line, a streamlit web appication will pop up. 

For each block, name the block, the sender, the receiver, and input the amount transacted. 

Once the necessary inputs are provided for each block, click "Add Block"

Each time a block is added and validated, the block will be added to the ledger below that keeps a record of each block, which begins with the first "genesis" block

Each record consists of record name or json, creator_id, previous hash, timestamp, and nonce count

User can click "Validate Chain" to check that each block in the chain was added correctly and store's previous blocks hash

User can also increas the validation level of difficulty and inspect the json contents of each block via a pop up window on the leftside pane
...
![alt=""](Images/blockchain_detail_fullpage.png)

![alt=""](Images/blockchain_detail.png)
...
![alt=""](Images/difficulty_and_blockinspection.png)
